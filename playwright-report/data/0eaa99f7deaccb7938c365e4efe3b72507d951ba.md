...
- Name: login2.demo.spec.ts >> login test
- Location: tests\login2.demo.spec.ts:3:5

# Error details

```
TypeError: username.goto is not a function
```
...
```ts
  ...
  2  | 
  3  | test('login test' , async ({page}) =>{ // page is fixture provided by playwright test runner or global variable
  4  |     await page.goto('https://www.saucedemo.com/');
  5  | 
  6  |     const username: Locator = page.locator('#user-name');
  7  |     const password: Locator = page.locator('#password');
  8  |     const loginButton: Locator = page.locator('#login-button');
  9  | 
  10 |     await username.fill('standard_user');
  11 |     await password.fill('secret_sauce');
  12 |     await loginButton.click();
  13 | 
> 14 |     username.goto('') //It will give compile time error.
     |              ^ TypeError: username.goto is not a function
  ...
  17 |     await expect(page.locator('.inventory_item')).toHaveCount(6);
  18 |     await page.pause(); // pause the test to see the result in the browser
  19 | })
```